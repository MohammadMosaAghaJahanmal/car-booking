...
type AuthValue={user:User|null;token:string|null;ready:boolean;signIn:(email:string,password:string)=>Promise<User>;signUp:(name:string,email:string,password:string)=>Promise<User>;signOut:()=>Promise<void>;updateUser:(user:User)=>Promise<void>};
...
export function AuthProvider({children}:React.PropsWithChildren){
 ...
 const signIn=async(email:string,password:string)=>{const{data}=await api.post('/auth/login',{email,password});await persist(data.token,data.user);return data.user as User};
 const signUp=async(name:string,email:string,password:string)=>{await api.post('/auth/register',{name,email,password});return await signIn(email,password)};
